    ...
    for style in all_styles(doc):
        ...
        if (text_properties := style.getElementsByType(TextProperties)) and (
            ...
            if parent in styles_cache:
                yield style.getAttribute("name"), styles_cache[parent].getAttribute("fontname")
        else:
...
def convert_styles(doc:OpenDocument) -> None:
    ...
    for style in all_styles(doc):
        ...
        if text_properties := style.getElementsByType(TextProperties):
            text_properties = text_properties[0]  # there's only one TextProperties in style
            if font_name := text_properties.getAttribute("fontname"):
                font_name = font_name.lower()
                if conversion := the_stupid_fonts.get(font_name):
                    text_properties.setAttribute("fontname", conversion.replacement_font)
...
def convert_document(doc:OpenDocument) -> None:
    ...
    for el in saxiter(doc.body):
        if el.__class__ is Text and el.parentNode.tagName.startswith("text:"):
...
def convert_file(file: Union[str, BinaryIO, Path], output_filename: Union[str, Path]) -> None:
    doc = load(file)
